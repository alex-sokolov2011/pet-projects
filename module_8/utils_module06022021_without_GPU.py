...
def auto_eda_cat_col(d_col,
    ...
    print(f'КУЗ во всем датасете:= {d_df[d_col].nunique()}')
    print('===')
    ...
    print(f'КУЗ в трейне:= {len(set_Train)}')
    print(f'КУЗ в тесте:= {len(set_Test)}')
    print('===')
    print('Список уникальных значений (СУЗ)')
    print(f'СУЗ в трейне:= {set_Train}')
    print(f'СУЗ в тесте:=  {set_Test}')
    print('===')
    print('Различия в уникальных значениях (РУЗ)')
    print(f'РУЗ в трейне (что есть в трейне и нет в тесте):= {set_Train.difference(set_Test)}')
    print(f'РУЗ в тесте (что есть в тесте и нет в трейне):= {set_Test.difference(set_Train)}')
    print('===')
    d_list_RUZ = list(set_Train.difference(set_Test))
    
    d_len_Train = len(temp_df_Train)
    if len(d_list_RUZ)>0:
        
        for uz in d_list_RUZ:
            print()
            d_len_uz = len(temp_df_Train[temp_df_Train[d_col] == uz])
            dolya = d_len_uz/d_len_Train*100
            if dolya < 0.1:
                udalit = 'Рекомендую удалить.'
            else:
                udalit = ''
...
def simple_plot_barh_procent(d_name_title, d_category_names, d_name_column, d_df):
    ...
    if len(d_category_names) != len(list_values):
        ...
        for (x, w, c_n, k) in zip(xcenters, widths, d_category_names, temp_df.keys()):
            ax.text(x,-0.05, str(round(w,1))+'%', fontsize=18, weight = 'bold', ha='center', va='center',
                    color=text_color)
